...
def reshape_for_broadcast(freqs_cis, x):
    # x shape is [b, h, n, d//2] (complex)
    # freqs_cis shape is [seq_len_full, d//2] (complex)
    ndim = x.ndim
    assert 0 <= 2 < ndim # seq_len dim is 2
    
    # Slice freqs_cis to the correct sequence length
    _freqs_cis = freqs_cis[: x.shape[2]] # [n, d//2]
    
    # Prepare shape for broadcasting. We want [1, 1, n, d//2]
    # to broadcast across batch and heads
    shape = [1] * ndim
    shape[2] = x.shape[2]   # Set seq_len dim
    shape[3] = x.shape[3]   # Set feature dim
    
    return _freqs_cis.view(*shape) # [1, 1, n, d//2]
...
def apply_rotary_pos_emb(q, k, freqs_cis):
    # q, k shape is [b, h, n, d] (real)
    # freqs_cis shape is [n_full, d // 2] (complex)
    
    # Reshape q, k for complex operations
    # q_ shape becomes [b, h, n, d//2, 2]
    q_ = q.float().reshape(*q.shape[:-1], -1, 2)
    k_ = k.float().reshape(*k.shape[:-1], -1, 2)
    
    # View as complex
    # q_ shape becomes [b, h, n, d//2]
    q_ = torch.view_as_complex(q_)
    k_ = torch.view_as_complex(k_)
    
    # freqs_cis is [n_full, d//2], slice and reshape it to [1, 1, n, d//2]
    freqs_cis = reshape_for_broadcast(freqs_cis, q_)

    # Apply rotation via complex multiplication
    # q_out shape [b, h, n, d//2] (complex)
    q_out_complex = q_ * freqs_cis
    k_out_complex = k_ * freqs_cis
    
    # View as real
    # q_out shape [b, h, n, d//2, 2]
    q_out = torch.view_as_real(q_out_complex)
    k_out = torch.view_as_real(k_out_complex)
